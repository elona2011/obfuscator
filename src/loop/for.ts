import { Node, ForStatement, Statement, BreakStatement, ContinueStatement } from 'estree'
import * as estraverse from 'estraverse'
import { generate } from 'escodegen'
import { parseScript } from 'esprima'
import { deepCopy } from '../utils/util'
import { CaseOptions, setNextNum, getCaseNum, getNextNum } from '../function/case'
import { getBreakStatement, getBlockStatement, getNextStep, createNewCase, newExpressionStatement } from '../utils/syntaxTree'
import { replaceNode, replaceNodeSkip, validateTypes } from '../utils/traverse'

interface ForOptions extends CaseOptions {
  firstStatement: ForStatement
}

function astFor({ switchCase, switchStatement, caseLen, firstStatement, varNames }: ForOptions) {
  ...
  replaceAndSkipFor(validateTypes(['BreakStatement'])(() => getNextStep(varNames[0], getNextNum(switchCase))))
  replaceAndSkipFor(validateTypes(['ContinueStatement'])(() => getNextStep(varNames[0], caseLen + 2)))

  //for.init
  newCase1.consequent = [<Statement>parseScript(`${varNames[0]}=${generate(firstStatement.test)}?${caseLen + 2}:${getNextNum(switchCase)}`).body[0], getBreakStatement()]
  ...
  if (firstStatement.body.type === 'BlockStatement') {
    ...
    newCase2.consequent = [<Statement>firstStatement.body].concat(newCase2.consequent)
  }
  ...
  if (firstStatement.init && firstStatement.init.type === 'AssignmentExpression') {
    ...
    switchCase.consequent.splice(0, 1, <Statement>firstStatement.init)
  } else {
